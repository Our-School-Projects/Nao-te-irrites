...
var localPose = []
var winPose = [0,0,0,0]
@onready var playerPath = [$Y1,$Y2,$Y3,$Y4]
	...
	if set.size() ==1:
		for y in loadPlayer.size():
			if set[0] == loadPlayer[y]:
				reset = 1
		if reset == 0:
			emit_signal("playDoneYellow",playerPos,playerTurn,diceFace,1,set[0])
		else:
			emit_signal("playDoneYellow",playerPos,playerTurn,diceFace,0,null)
	else:
		emit_signal("playDoneYellow",playerPos,playerTurn,diceFace,0,null)
